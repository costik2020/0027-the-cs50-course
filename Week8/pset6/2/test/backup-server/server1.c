...
if (pdir==NULL)
	{
	printf("Unable to open the directory pointer opendir() \n");
	return NULL;
	}
...
while ((sdirectories=readdir(pdir) )!= NULL   )
	...
	if (strcmp(name,"index.html")==0)                        
		...
		//debug
		//printf("3BUG index newpath=%s \n",newpath);
		strcat(newpath,"index.html");
		//debug
		//printf("1BUG index newpath=%s \n",newpath);
		return newpath;
		}
...
void interpret(const char* path, const char* query)
{
    ...
    {
    //THE 500 INTERNAL ERROR IS HERE SOMEWHERE
    // The load returns false it should return true
    printf("file=%p \n",file);
    printf("content=%s \n",content);
    printf("length=%lu \n",length);
...
int nrOfItems=0;
//debug
storebytes=malloc(sizeof(char)*100);
printf("storebytes=%p \n",storebytes);	

while (true)
	{
	// Read 8 bits into the buffer from the file
	//typedef char BYTE; is at the top of .c file
	...
	if (nrOfItems > 0)
		...
		if (storebytes==NULL)
			{printf("bug22222222222222222222");
